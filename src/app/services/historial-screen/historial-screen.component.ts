...
@Component({
  ...
  templateUrl: './historial-screen.component.html',
  styleUrls: ['./historial-screen.component.scss']
})
export class HistorialScreenComponent implements OnInit {
  ...
  //Para la tabla
  displayedColumns: string[] = ['id', 'fecha_prestamo', 'fecha_devol_esperada', 'fecha_devol_real', 'id_bibliotecario', 'id_libro', 'id_cliente', 'estado', 'editar', 'eliminar'];
  ...
  public initPaginator(){
   ...
   public obtenerHistorial(){
    this.prestamosService.obtenerListaHitorial().subscribe(
      (response)=>{
        this.lista_prestamos = response;
        console.log("Lista hitorial: ", this.lista_prestamos);
        ...
        alert("No se pudo obtener la lista del hitorial");
      }
    );
  }
